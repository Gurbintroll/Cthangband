...
namespace Cthangband
{
    ...
    internal class Island
    {
        ...
        public void MakeIslandContours()
        {
            ...
            {
                reject = false;
                var perlinNoise = new PerlinNoise(Program.Rng.RandomLessThan(int.MaxValue - 1));
                ...
                for (var row = 0; row < mapHeight; row++)
                {
                    ...
                    {
                        var v = perlinNoise.Noise(10 * col * widthDivisor, 10 * row * heightDivisor, -0.5);
                        v = (v + 1) / 2;
                        var dX = Math.Abs(col - (mapWidth / 2)) * widthDivisor;
                        var dY = Math.Abs(row - (mapHeight / 2)) * heightDivisor;
